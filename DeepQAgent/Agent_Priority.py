...
BUFFER_SIZE = int(3*1e3)# size of the memory buffer
...
TAU = 1.75e-3           # for soft update of target parameters
LR = 3e-4  #[2e-4,5e-4,1e-3,1e-2,5e-3]               # learning rate 
UPDATE_TSTEP = 6       # number of time_steps after which we update the QNetwork
...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, device):
        ...
        # get the optimizer
        """
        self.optimizer = optim.RMSprop( self.qnet_local.parameters(), lr=LR, alpha=0.99, eps=5e-08,
                                        weight_decay = 1e-4, momentum=0.01, centered=False
                                      )
        """
        self.optimizer = optim.Adam( self.qnet_local.parameters(), lr=LR )
        
        """
        RMS-PROP:[weight_decay = 1e-4, eps = 5e-8, lr = 5e-4](ver: 2), [weight_decay = 1e-3, eps = 5e-8, lr = 1e-3](ver: 1)
        ADAM: lr = [1e-4], lr = [2e-4], lr = [3e-4]
        
        (verL 4) -> Deeper NN arch -> 64-128-64-128-64-16
        (ver: 3) -> larger NN arch. -> 128-256-64-16
        (ver: 2) -> has smaller NN architecture -> 64-128-64-16
        (ver: 1) -> 64-128-128-64
        """
    ...
    def learn( self, experiences, GAMMA, hparam_b = 0.001):
        """
